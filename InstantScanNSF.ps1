...
# Function to clean up existing jobs
function Initial-Cleanup {
    Write-Log "Performing initial cleanup of existing jobs..." "INFO"
    ...
    if ($ExistingJobs) {
        foreach ($Job in $ExistingJobs) {
            Write-Log "Stopping and removing existing job: $($Job.Name) (ID: $($Job.Id))" "WARNING"
            Stop-Job $Job -ErrorAction SilentlyContinue
            Remove-Job $Job -Force -ErrorAction SilentlyContinue
            Write-Log "Stopped and removed job: $($Job.Name)" "DEBUG"
        }
        Write-Log "Initial cleanup completed." "INFO"
    } else {
        Write-Log "No existing jobs found. Proceeding..." "INFO"
    }
}
...
function Log-ProcessedFile {
    ...
    if ($EnableProcessedFilesLog) {
        try {
            ...
            Write-Log "Logged processed file: $File ($Status)" "DEBUG"
        } catch {
...
function Cleanup-Script {
    Write-Log "Cleaning up background jobs and event subscriptions." "INFO"
    ...
    try {
        ...
        foreach ($Job in $JobsToCleanup) {
            Write-Log "Stopping job: $($Job.Name) (ID: $($Job.Id))" "DEBUG"
        ...
        Write-Log "Unregistering event subscribers." "DEBUG"
        Get-EventSubscriber | ForEach-Object {
            if ($_.Id) {
                try {
                    Unregister-Event -SubscriptionId $_.Id -Force -ErrorAction SilentlyContinue
                    Write-Log "Unregistered event subscriber: $($_.Id)" "DEBUG"
                } catch {
                    Write-Log "Failed to unregister event subscriber: $_" "ERROR"
                }
            } else {
                Write-Log "Skipped unregistering an event subscriber with null SubscriptionId." "WARNING"
            }
        }

        # Unregister FileSystemWatcher events if any
        if ($Watcher) {
            Write-Log "Unregistering FileSystemWatcher events." "DEBUG"
            try {
                Unregister-Event -SourceIdentifier FileCreatedEvent -ErrorAction SilentlyContinue
                Write-Log "Unregistered FileSystemWatcher events." "DEBUG"
            } catch {
                Write-Log "Failed to unregister FileSystemWatcher events: $_" "ERROR"
            }
        }

        # Remove global variables
        Write-Log "Removing global variables." "DEBUG"
        ...
        foreach ($var in $globalVariables) {
            if (Get-Variable -Name $var -Scope Global -ErrorAction SilentlyContinue) {
                ...
                Write-Log "Removed global variable: $var" "DEBUG"
            }
        }

        Write-Log "Cleanup completed successfully." "INFO"
    } catch {
        Write-Log "An error occurred during cleanup: $_" "ERROR"
    }
}

# Register the cleanup function to run on script exit
Register-EngineEvent PowerShell.Exiting -Action { Cleanup-Script } | Out-Null
...
if ($cancelKeyPressEvent) {
    ...
    try {
        ...
        Write-Log "CancelKeyPress handler registered successfully." "INFO"
    } catch {
        Write-Log "Failed to register CancelKeyPress handler: $_" "ERROR"
    }
} else {
    Write-Log "CancelKeyPress event is not available in this PowerShell host." "WARNING"
}
...
# Log starting of workers
Write-Log "Starting worker threads..." "INFO"
...
function Start-WorkerJob {
    ...
    Write-Log "Started Worker Job for file: $FilePath with Job ID: $($Job.Id)" "INFO"
}
...
if ($LogProcessorJob.State -eq 'Running') {
    Write-Log "Log processor job started successfully." "INFO"
} else {
    Write-Log "Log processor job failed to start. State: $($LogProcessorJob.State)" "ERROR"
}
...
$FileCreatedEvent = Register-ObjectEvent $Watcher Created -SourceIdentifier FileCreatedEvent -Action {
    try {
        ...
        Write-Log "FileSystemWatcher triggered for file: $FilePath" "INFO"

        # Get the current time
        $CurrentTime = Get-Date

        # Log the current time
        Write-Log "Current Time: $CurrentTime" "DEBUG"

        # Define a timeframe to consider events as duplicates (e.g., 5 seconds)
        $DuplicateTimeframe = [TimeSpan]::FromSeconds(5)
        Write-Log "Duplicate Timeframe: $DuplicateTimeframe" "DEBUG"
        ...
        if ($KeysToRemove.Count -gt 0) {
            Write-Log "Keys to Remove: $($KeysToRemove -join ', ')" "DEBUG"
        } else {
            Write-Log "No keys to remove based on the timeframe." "DEBUG"
        }

        # Remove old entries
        foreach ($Key in $KeysToRemove) {
            $Global:ProcessedFiles.Remove($Key) | Out-Null
            Write-Log "Removed old entry from ProcessedFiles: $Key" "DEBUG"
        }

        # Check if the file was recently processed
        if (-not $Global:ProcessedFiles.ContainsKey($FilePath)) {
            Write-Log "Enqueued new NSF file: $FilePath" "INFO"
            $Global:Queue.Enqueue($FilePath)
            $Global:ProcessedFiles[$FilePath] = $CurrentTime
            Write-Log "Added $FilePath to ProcessedFiles with timestamp $CurrentTime" "DEBUG"
        } else {
            Write-Log "Duplicate event detected for file: $FilePath. Ignoring." "DEBUG"
        }
    } catch {
        Write-Log "Error in FileCreatedEvent handler: $_" "ERROR"
    }
}
...
if ($EnableHeartbeat) {
    Write-Log "Heartbeat mechanism enabled." "INFO"
    ...
    Write-Log "Heartbeat ThreadJob started." "INFO"
}
...
if ($EnableSelfCheck) {
    Write-Log "Self-check mechanism enabled." "INFO"
    ...
    Write-Log "Self-check ThreadJob started." "INFO"
}
...
try {
    Write-Log "Monitoring directory: $Folder" "INFO"
    Write-Log "Script started successfully." "INFO"
